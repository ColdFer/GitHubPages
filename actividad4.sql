...
EXPLAIN ANALYZE 
SELECT * FROM Inscripciones WHERE dni_estudiante = 500;
DROP INDEX IF EXISTS idx_insc_estudiante;
DROP INDEX IF EXISTS idx_insc_materia;
DROP INDEX IF EXISTS idx_persona_nombre;
...
SELECT * FROM VM_EstadisticasMateria ORDER BY Tasa_Aprobacion DESC;


-- Vista 1: Ocultar datos sensibles
CREATE OR REPLACE VIEW V_ResumenAcademico AS
SELECT 
    p.nombre        AS Estudiante,
    m.nombre_materia AS Materia,
...
-- Crear Vista Materializada (se almacena en disco)
CREATE MATERIALIZED VIEW VM_EstadisticasMateria AS
SELECT m.nombre_materia,
    COUNT(i.id_inscripcion) AS Inscritos,
    ROUND(AVG(i.nota), 2)  AS Promedio,
    COUNT(CASE WHEN i.nota >= 51 THEN 1 END) * 100.0
       / COUNT(i.id_inscripcion) AS Tasa_Aprobacion
FROM Materia m
LEFT JOIN Inscripciones i ON m.id_materia = i.id_materia
GROUP BY m.nombre_materia;

-- OBLIGATORIO: Actualizar tras cambios en datos
REFRESH MATERIALIZED VIEW VM_EstadisticasMateria;
SELECT * FROM VM_EstadisticasMateria ORDER BY Tasa_Aprobacion DESC;
--David Fernando Mayorga Barco
